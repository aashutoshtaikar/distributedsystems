...
func fcheckmsg(message string) string{
		...
		mutex.Lock()
		if strings.Contains(message,"RequestVote 8000") {
				if(votesen == "") {
					votesen="8000"
					message = "Vote"
					state = "follower"
			...
			leader="8004"
			message="Ack Leader"
		} else if strings.Contains(message,"heartbeat") && leader != "8001"{
