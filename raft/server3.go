...
func fcheckmsg(message string) string{
		...
		if strings.Contains(message,"RequestVote 8001") {
		...
		} else if strings.Contains(message,"RequestVote 8000") {
				if(votesen == "") {
					votesen="8000"
					message = "Vote"
					state = "follower"
					fmt.Println("voted for 8000")
				} else {
					message = "NoVote"
					fmt.Println("Cannot vote for 8000,as previously voted for ",votesen)
				}
			...
			leader="8002"
			message="Ack Leader"
		} else if strings.Contains(message,"Leader:8000") {
			...
			leader="8004"
			message="Ack Leader"
		} else if strings.Contains(message,"heartbeat") && leader != "8003"{
